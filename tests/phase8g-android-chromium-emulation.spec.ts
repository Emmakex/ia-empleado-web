...
test("Android Chromium emulation renders critical mobile routes without runtime errors or overflow", async ({
  browserName,
  page,
}) => {
  ...
  for (const route of criticalRoutes) {
    await test.step(route, async () => {
      ...
      expect(response?.ok(), `${route} must return success in Android Chromium emulation`).toBeTruthy();
...
test("Android Chromium emulation keeps the canonical Home character cards inside the viewport", async ({ page }) => {
...
test("Android Chromium emulation supports touch navigation and the primary mobile CTA handoff", async ({ page }) => {
  const pageErrors: string[] = [];
  page.on("pageerror", (error) => pageErrors.push(error.message));
