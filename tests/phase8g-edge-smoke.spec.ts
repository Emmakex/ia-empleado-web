...
test("Microsoft Edge renders critical ES/EN commercial routes without page errors", async ({
  browserName,
  page,
}) => {
  ...
  expect(await page.evaluate(() => navigator.userAgent)).toContain("Edg/");

  for (const route of criticalRoutes) {
    await test.step(route, async () => {
      const response = await page.goto(route, { waitUntil: "domcontentloaded" });
      expect(response?.ok(), `${route} must return success in Microsoft Edge`).toBeTruthy();
...
test("Microsoft Edge keeps Home stable at the Phase 8G 768px entry-risk viewport", async ({ page }) => {
  ...
  expect(metrics.scrollWidth, "Home must not overflow horizontally at 768px in Microsoft Edge").toBeLessThanOrEqual(
...
test("Microsoft Edge mobile navigation traps and restores keyboard focus", async ({ page }) => {
