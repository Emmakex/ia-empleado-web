...
const files = {
  ...
  structuredNavigation: "tests/phase8f-structured-navigation.spec.ts",
...
const structuredNavigation = read(files.structuredNavigation);
...
for (const phrase of [
  ...
  if (!structuredNavigation.includes(phrase)) {
    throw new Error(`Phase 8F structured/navigation production gate missing phrase: ${phrase}`);
  }
}
...
console.log(`Web Phase 8F release gate OK: ${marker} is active in ES/EN; permanent Phase 8A-8E regressions, Lighthouse, runtime SEO and structured-data/navigation coverage remain protected.`);
